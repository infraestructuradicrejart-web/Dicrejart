...
// ============================================================
// SUBIDA AUTOMÁTICA DE EVIDENCIAS/ARCHIVOS AL NAS (SYNOLOGY)
// ============================================================
// Evidencia de Actividad, evidencia de Auditoría de Calidad, y Ayudas Visuales del
// lienzo (imágenes/PDFs/modelos 3D) se suben aquí en vez de a Firebase Storage — con
// respaldo automático a Firebase si el NAS no responde, y reintento de migración
// (nasMigrationRetryCheck) hasta que quede en el NAS. La cuenta de Synology usada aquí
// debe tener acceso de escritura SOLO a NAS_ROOT_PATH — nunca la cuenta admin del NAS.

const NAS_URL = defineSecret('NAS_URL');
const NAS_USER = defineSecret('NAS_USER');
const NAS_PASS = defineSecret('NAS_PASS');
const NAS_ROOT_PATH = defineString('NAS_ROOT_PATH', { default: '/evidencias' });
const NAS_SECRETS = [NAS_URL, NAS_USER, NAS_PASS];
...
/** Lee la ruta ya cacheada de una carpeta NAS, si `resolveNasFolder` ya la resolvió antes. */
...
const buildNasFolderCacheUpdate = (category, areaId, path) =>
  category === 'fabricacion'
    ? { [`nasFolderPaths.fabricacion.${areaId || 'general'}`]: path }
    : { [`nasFolderPaths.${category}`]: path };

/**
 * Resuelve (y cachea) la carpeta del NAS donde debe ir un archivo — la parte
 * "inteligente": primer nivel por categoría (Calidad / nombre del Área / Diseño y
 * Planos), segundo nivel por Juego (o Proyecto si no hay Juego). Si ya se resolvió antes
 * para ese mismo Juego/Proyecto+categoría, se reutiliza sin tocar el NAS.
 */
async function resolveNasFolder(nasUrl, sid, db, category, ctx) {
  ...
  const bucketPath = await sc.ensureFolder(nasUrl, sid, NAS_ROOT_PATH.value(), sanitizeFolderName(bucketName));
  const finalPath = await sc.ensureFolder(nasUrl, sid, bucketPath, subFolderName);
...
/**
 * Sube un archivo (recibido en base64 desde la app, ya autenticada) al NAS, organizado
 * por categoría/Área/Juego. Para evidencia de Actividad/Auditoría regresa el link
 * público de DSM para compartir (solo se usa con un botón "Abrir"); para Ayudas
 * Visuales (`category: 'diseno'`) regresa la URL de nuestro propio proxy
 * (`nasFileProxy`) para garantizar que la vista previa embebida siga funcionando.
 */
exports.uploadToNas = onCall({ cors: true, secrets: NAS_SECRETS, timeoutSeconds: 120, memory: '512MiB' }, async (request) => {
  const { auth, data } = request;
  if (!auth) throw new HttpsError('unauthenticated', 'Debes iniciar sesión.');

  const { fileBase64, fileName, mimeType, category, areaId, areaName, gameId, gameName, projectId, projectName } = data || {};
  if (!fileBase64 || !fileName || !category) {
    throw new HttpsError('invalid-argument', 'Falta el archivo, el nombre o la categoría.');
  }

  const nasUrl = NAS_URL.value();
  const db = admin.firestore();
  let sid = null;
  try {
    sid = await sc.login(nasUrl, NAS_USER.value(), NAS_PASS.value());
    const folderPath = await resolveNasFolder(nasUrl, sid, db, category, { areaId, areaName, gameId, gameName, projectId, projectName });
    const finalName = `${Date.now()}_${sanitizeFileName(fileName)}`;
    const buffer = Buffer.from(fileBase64, 'base64');
    const filePath = await sc.uploadFile(nasUrl, sid, folderPath, finalName, buffer, mimeType);

    const url =
      category === 'diseno'
        ? `${APP_BASE_URL.value()}/api/nasfile?path=${encodeURIComponent(filePath)}`
        : await sc.createShareLink(nasUrl, sid, filePath);

    return { ok: true, url, nasPath: filePath };
  } catch (error) {
    console.error('Error al subir archivo al NAS:', error);
    throw new HttpsError('internal', error.message || 'No se pudo subir el archivo al NAS.');
  } finally {
    if (sid) await sc.logout(nasUrl, sid);
  }
});

/** Borra un archivo del NAS — best-effort (no crítico, no debe tumbar la acción del usuario si falla). */
exports.deleteFromNas = onCall({ cors: true, secrets: NAS_SECRETS }, async (request) => {
  const { auth, data } = request;
  if (!auth) throw new HttpsError('unauthenticated', 'Debes iniciar sesión.');
  const { nasPath } = data || {};
  if (!nasPath) return { ok: false, error: 'Falta la ruta del archivo.' };

  const nasUrl = NAS_URL.value();
  let sid = null;
  try {
    sid = await sc.login(nasUrl, NAS_USER.value(), NAS_PASS.value());
    await sc.deleteFile(nasUrl, sid, nasPath);
    return { ok: true };
  } catch (error) {
    console.error('Error al borrar archivo del NAS:', error);
...
/**
 * Sirve el archivo crudo de una Ayuda Visual guardada en el NAS — necesario porque el
 * link público para compartir de Synology abre una página HTML de DSM, no el archivo
 * crudo, así que no sirve para `<img src>`/`<iframe>`. Publicada en Hosting como
 * `/api/nasfile` (ver firebase.json) por la misma razón que `checkSystemEmpty`: la
 * política de la organización no permite invocador público directo en `onRequest`.
 */
exports.nasFileProxy = onRequest({ cors: true, secrets: NAS_SECRETS }, async (req, res) => {
  const filePath = req.query.path;
  if (!filePath) {
    res.status(400).send('Falta el parámetro path.');
    return;
  }
  ...
  try {
    sid = await sc.login(nasUrl, NAS_USER.value(), NAS_PASS.value());
    const fileRes = await sc.downloadFile(nasUrl, sid, filePath);
    const buffer = Buffer.from(await fileRes.arrayBuffer());
    res.set('Content-Type', fileRes.headers.get('content-type') || 'application/octet-stream');
    res.set('Cache-Control', 'public, max-age=3600');
    res.status(200).send(buffer);
  } catch (error) {
    console.error('Error al servir archivo del NAS:', error);
    res.status(502).send('No se pudo obtener el archivo del NAS.');
  } finally {
    if (sid) await sc.logout(nasUrl, sid);
  }
});
...
/**
 * Aplica la URL ya migrada al NAS sobre el registro real (Actividad, veredicto de
 * Auditoría por Juego+Área o por Proyecto, o nodo `recurso` de un lienzo).
 */
async function applyMigratedNasUrl(db, targetType, targetRef, url, nasPath) {
  if (targetType === 'actividad') {
    ...
    // El nodo vive dentro del arreglo `nodes` del documento `lienzos/{lienzoId}` — no se
    // puede actualizar un elemento de arreglo por id con dot-path, hay que leer, mapear
    // y reescribir el arreglo completo.
    ...
    const nextNodes = nodes.map((n) => {
      if (n.id !== targetRef.nodeId) return n;
      return { ...n, draftFields: { ...n.draftFields, fileData: { ...n.draftFields?.fileData, url, nasPath, storagePath: null } } };
    });
...
/**
 * Corre cada 30 minutos: procesa `nasMigrationQueue` (archivos que quedaron
 * temporalmente en Firebase Storage porque el NAS no respondió al subirlos) e intenta
 * moverlos al NAS. Reintenta indefinidamente — no hay límite de intentos, tal como se
 * pidió — dejando `attempts`/`lastError` en el documento para diagnóstico.
 */
exports.nasMigrationRetryCheck = onSchedule({ schedule: 'every 30 minutes', secrets: NAS_SECRETS }, async () => {
  const db = admin.firestore();
  ...
  try {
    ...
    for (const docSnap of pendingSnap.docs) {
      ...
      try {
        ...
        const folderPath = await resolveNasFolder(nasUrl, sid, db, item.category, item);
        const finalName = `${Date.now()}_${sanitizeFileName(item.fileName)}`;
        const filePath = await sc.uploadFile(nasUrl, sid, folderPath, finalName, buffer, item.mimeType);

        const url =
          item.category === 'diseno'
            ? `${APP_BASE_URL.value()}/api/nasfile?path=${encodeURIComponent(filePath)}`
            : await sc.createShareLink(nasUrl, sid, filePath);

        await applyMigratedNasUrl(db, item.targetType, item.targetRef, url, filePath);
        ...
        console.error(`Error migrando ${docSnap.id} al NAS:`, itemError);
        await docSnap.ref
